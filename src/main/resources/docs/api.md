...
说明：
    ...
    2. 只有管理员角色用户可以访问
    3. 用户id和用户角色由后端从token中解析，前端不需要传userId或userRole
    4. 该接口只处理新建作品审核，不处理小说信息变更和章节审核
    5. auditStatus只能传1或2；1表示审核通过，2表示审核驳回
    6. 只有待审核且未应用的新建作品审核记录可以被更新
    7. 审核通过后，会根据审核记录创建正式作品，并将新作品id回写到审核记录bookId
    8. 审核驳回后，只更新审核状态、审核备注和审核时间，不会创建正式作品
    9. 审核成功后会记录审核管理员、审核备注和审核时间
    10. 审核通过并创建正式作品成功后，审核记录会标记为已应用
```

## 管理员新书审核详情查询
```text
请求路径：
/api/admin/{auditId}/info/create?auditStatus=0
请求方式：
    GET
请求头：
    Authorization: Bearer token值
...
authorId -> 作家id
authorName -> 作家笔名
auditAdminName -> 审核管理员昵称，未审核时为null
bookIntro -> 新建作品简介
coverUrl -> 新建作品封面链接
auditRemark -> 审核备注，无备注时为null
auditType -> 审核类型（1-新建作品）
auditStatus -> 审核状态（0-待审核，1-已通过，2-已驳回）
subTime -> 提交时间
auditTime -> 审核时间，未审核时为null
异常响应：
...
说明：
    ...
    2. 只有管理员角色用户可以访问
    3. 用户id和用户角色由后端从token中解析，前端不需要传userId或userRole
    4. 该接口只查询新建作品审核详情，不查询小说信息变更和章节审核详情
    5. auditStatus为0时表示待审核，1表示已通过，2表示已驳回
    6. auditId和auditStatus必须同时匹配才会返回审核详情
    7. 待审核记录的auditAdminName和auditTime通常为null
    8. 审核记录不存在或不匹配时返回404
```
...
```text
请求路径：
/api/admin/{auditId}/info/chapter?auditStatus=0
请求方式：
    GET
请求头：
    Authorization: Bearer token值
...
说明：
    ...
    2. 只有管理员角色用户可以访问
    3. 用户id和用户角色由后端从token中解析，前端不需要传userId或userRole
    4. 该接口只查询章节审核详情，不查询小说信息变更和新书审核详情
    5. auditStatus为0时表示待审核，1表示已通过，2表示已驳回
    6. auditId和auditStatus必须同时匹配才会返回审核详情
    7. 待审核记录的auditAdminName、auditRemark和auditTime通常为null
    8. 审核记录不存在或不匹配时返回404
```
...
请求体：
    {
        "title": "系统维护通知更新",
        "announcementType": 0,
        "content": "系统维护时间调整为今晚23:00，请提前保存数据。",
        "publishStatus": 0
    }
title -> 公告标题，非必传；传入时不能为空字符串
announcementType -> 公告类型，非必传；0表示全体，1表示作者，2表示读者
content -> 公告内容，非必传；传入时不能为空字符串
publishStatus -> 发布状态，非必传；0表示发布，1表示草稿，2表示下架
响应数据：
    {
        "code": 200,
        "message": "操作成功",
...
说明：
    1. 该接口需要登录后调用
    2. 只有管理员角色用户可以访问
    3. announcementId为路径参数，表示要修改的公告id
    4. 请求体中至少传入title、announcementType、content、publishStatus中的一个字段
    5. title和content传入时不能为空字符串或空白字符串
    6. announcementType只能传0、1、2；0表示全体，1表示作者，2表示读者
...
```text
请求路径：
/api/author/list/announcement?pageNum=1&pageSize=10
请求方式：
    GET
请求头：
    Authorization: Bearer token值

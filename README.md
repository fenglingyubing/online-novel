...
项目围绕读者端、作家端、管理员端三类角色设计，覆盖小说浏览阅读、书架收藏、阅读历史、评论互动、作家创作、章节管理、内容审核、公告发布和首页推荐等核心流程，形成从内容生产到内容消费的完整闭环。

### 2. JWT 登录态与并发注册控制

系统使用 JWT 作为接口访问凭证，并将登录 Token 写入 Redis 维护服务端登录态。拦截器在请求进入业务逻辑前完成 Token 格式校验、JWT 合法性校验、Redis 登录态校验和用户状态校验；校验通过后将用户身份写入线程上下文，便于后续业务获取当前登录用户。

注册时基于用户名加 Redis 短期锁，减少同一用户名并发注册造成的重复写入；同时通过数据库用户名唯一索引兜底，保证并发场景下的数据一致性。

### 3. Redis 缓冲高频阅读历史

阅读进度更新频率高，如果每次章节阅读都直接写入 MySQL，容易产生大量重复更新。本项目将最新阅读进度先写入 Redis Hash，并使用 Redis ZSet 记录待同步的用户-小说标识，再由定时任务批量落库。

同步失败的数据会重新加入待同步队列，等待下一轮任务重试，从而在减少数据库写入压力的同时保证阅读历史最终可落库。

### 4. 内容发布审核机制

系统将作家提交内容与用户端展示解耦，支持新书审核、小说信息变更审核和章节审核。作家创建或修改内容后先进入待审核状态，由管理员审核通过后再对外展示，避免未审核内容直接进入阅读端。

### 5. 面向查询场景的表结构与索引

数据库围绕用户、作家、小说、章节、书架、评论、审核、公告、推荐、阅读历史等业务实体建模，并针对高频查询和唯一性约束设计索引，例如用户名唯一索引、用户书架唯一索引、阅读历史用户-小说唯一索引，以及小说分类、发布状态、更新时间等组合查询索引。

这些设计能减少重复数据，提升列表查询、阅读进度更新、书架操作和推荐展示等场景的查询效率。

### 6. 文件上传与部署配置完整

系统接入阿里云 OSS 存储用户头像和小说封面，避免上传文件依赖单台应用服务器；同时提供 Dockerfile、Docker Compose 配置和部署文档，配合环境变量管理数据库、JWT 和 OSS 等敏感配置，便于本地运行和线上部署。
